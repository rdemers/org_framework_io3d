 ...
 *
 * Description
 * - Tests of simple triangle with API from Vulkan library.
 * *****************************************************************************
 */
...
void getInstanceExtensions() {
  vkEnumerateInstanceExtensionProperties =
      Pointer<NativeFunction<VkEnumerateInstanceExtensionPropertiesNative>>
...
void getPhysicalDevice() {
  ...
  Logger().log(Level.info,'physical devices count $physicalDeviceCount');
  ...
  for (var i = 0; i < physicalDeviceCount; i++) {
    ...
    if (physicalDevice == nullptr && isDeviceSuitable(device)) {
      ...
      Logger().log(Level.info,'pick [$deviceName]');
    }
  }
}
...
void createGraphicsPipeline() {
  final vertShaderCode = File('./asserts/main.vert.sprv').readAsBytesSync();
  final fragShaderCode = File('./asserts/main.frag.sprv').readAsBytesSync();
...
int errorCallback(int error, Pointer<Uint8> description) {
  Logger().log(Level.info,'GLFW $error ${description.cast<Utf8>().toDartString()}');
  return 0;
}
